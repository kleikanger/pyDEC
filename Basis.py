...
class Basis():
    ...
    def __print_dalton_basisfile(self, filename):
        '''
        ...
        f.write('$ from the input file %s.\n' % self.infile_name)
        print('Writing basis in DALTON format to file: %s' % filename)
    ...
    def __print_elem_to_dalton_basisfile(self, elem, f):
        '''
        @brief Print chemical element basis to DALTON basisfile.
        ...
        for l in lset:
            ...
            # XXX what is the last integer??
            f.write('%i %i %i \n'% (total_n_exp, len(aos_set), 0))

            # organize and print exponents
    ...
    def __print_elem_to_cp2k_basisfile(self, elem, f):
        ...
        for _ in range(n_unique_exp):
            ...
            for j in range(len(e)):
                ostr = str(e[j]).ljust(12)
                f.write('%s ' % ostr)
                for l in range(lmax-lmin+1):
                    for k in range(lens[l]):
                        ostr = str(c_l[l][k][j]).ljust(12)
                        f.write('%s ' % ostr)
                f.write('\n')
    ...
    def __get_aoorder_dalton(self, atoms):
        ...
        for atom in atoms:
            ...
            tags, n = self.__get_order_of_dalton_aos_tags(elem)
...
basis.print_basisfile('dalton_STO-3G__2', 'DALTON')
